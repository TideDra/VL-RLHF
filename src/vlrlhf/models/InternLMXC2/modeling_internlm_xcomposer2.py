...
class InternLMXComposer2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def encode_text(self, text, add_special_tokens=False):
        token = self.tokenizer(text, return_tensors="pt", add_special_tokens=add_special_tokens).input_ids.to(
            self.device
        )
    ...
    def img2emb(self, image):
        ...
        torch_dtype = self.vision_proj[0].weight.dtype
        img_embeds = self.vision_proj(img_embeds.to(torch_dtype))
    ...
    def prompt_wrap(self, img_embeds, prompt):
        ...
        p_before_tokens = self.tokenizer(p_before, return_tensors="pt", add_special_tokens=True).to(img_embeds.device)
        ...
        wrapped_target = (
            torch.ones(batch_size, wrapped_img_embeds.shape[1], dtype=torch.long).to(img_embeds.device) * -100
        )
            ...
            for idx, part in enumerate(parts):
                if len(part) > 0:
                    part_tokens = self.tokenizer(
                        part, return_tensors="pt", padding="longest", add_special_tokens=need_bos
                    ).to(self.device)
                    if need_bos:
                        need_bos = False
