...
@dataclass
class BaseModelOutputWithPastandImgPos(BaseModelOutputWithPast):
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if fake_images is not None:
            ...
            img_pos_map = torch.zeros(hidden_states.size()[:2])
            for idx, (i, a, b) in enumerate(img_pos):
                hidden_states[i][a + 1 : b] = images[idx]
                img_pos_map[i][a + 1 : b] = 1
            img_pos_map = img_pos_map.to(torch.bool)
        ...
        return BaseModelOutputWithPastandImgPos(
            last_hidden_state=hidden_states,
            past_key_values=presents,
            hidden_states=all_hidden_states,
            attentions=all_self_attentions,
            image_position_map=img_pos_map,
        )
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithPastandImgPos(
            ...
            image_position_map=transformer_outputs.image_position_map,
        )
