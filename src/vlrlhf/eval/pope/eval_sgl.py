...
import os
import argparse
from ..utils import run_vqa_sgl, get_model_cache, log_data_to_mysql
...
def collator(batch):
    inputs = [dict(image_path=b["image"], question=b["prompt"]) for b in batch]
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(file_root):
        for file in files:
            ...
            results = run_vqa_sgl(model_path, dataset, collator, processor_path, batch_size, endpoint)
            ...
            if args.report_to_mysql:
                try:
                    ...
                    logger.error(f"Failed to log data to mysql: {e}")
