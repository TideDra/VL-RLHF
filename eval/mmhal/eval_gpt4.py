...
import json
import time
from GPTFactory import smart_build_factory,GPTFactory
...
if __name__ == '__main__':
    ...
    parser.add_argument('--api_info_path', type=str, required=True)
    ...
    factory:GPTFactory = smart_build_factory(api_info,model='gpt-4',worker_num=worker_per_api*len(api_info),rpm=240,tpm=40000,temperature=0,max_tokens=4096)
    # load json file
    ...
    responses = [ None for _ in range(len(records))]
    inputs = []
    for i, record in enumerate(records):
        ...
        inputs.append({"prompt":input_text,"id":i})

    results = factory.run_task(inputs)
