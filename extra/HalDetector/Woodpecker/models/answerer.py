...
def get_answer_or_prepare(processor, model, img_path, qs):
    if batch.get((img_path,qs), None) is None:
        prompt = processor.format_multimodal_prompt(qs, img_path)
        batch[(img_path,qs)] = {'prompt': prompt, 'img_path':img_path}
        return None
    else:
        return batch[(img_path,qs)].get('output', None)

def process_batch(processor, model,generation_config):
    prompts = []
    img_paths = []
    ...
    inputs = processor(texts=prompts, images_path=img_paths, padding_side='left')
    inputs.to('cuda:0')
...
def get_all_answers(processor, model, entity_list, qs, ent_info, input_img_path, cur_answers):
    ...
    if len(entity_list)>1 or 'where' in qs.lower() or any([ent not in ent_info for ent in entity_list]):
        ...
        answer = get_answer_or_prepare(processor, model, input_img_path, qs) 
        ...
        for idx, img_path in enumerate(ent_info[entity]['crop_path']):
            ...
            answer = get_answer_or_prepare(processor, model, img_path, qs)
...
class Answerer:
    ...
    def __init__(self, val_model_path):
        
        self.processor = MyAutoProcessor.from_pretrained(val_model_path)
        self.processor.infer()
        self.model = MyAutoModel.from_pretrained(val_model_path, torch_dtype=torch.bfloat16).to('cuda:0')
    ...
    def generate_answers(self, sample: Dict):
        ...
        process_batch(self.processor, self.model,self.generation_config)
        all_answers = []
        for gen_qs in generated_qs:
            # border case: no question asked.
            if len(gen_qs) == 0:
