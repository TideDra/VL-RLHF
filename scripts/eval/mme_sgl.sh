...
DATA_DIR=data_dir/MME.tsv
workdir=./eval/mme/results/${TAG}
...
python -m vlrlhf.eval.mme.eval_sgl \
    --data_root $DATA_DIR \
...
python -m vlmeval.evaluate.yes_or_no  ${workdir}/${TAG}.xlsx --dataset MME

eval "source ${conda_path} ${vlrlhf_env_name}"
python -m vlrlhf.eval.mme.calculate \
